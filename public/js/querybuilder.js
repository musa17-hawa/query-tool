...
(function (global) {
  ...
  const OPERATORS = [
    { id: 'eq',        label: 'is',                  arity: 1, types: ['any'],
      sql: (col, vals) => `${col} = ${literal(vals[0])}` },
    { id: 'neq',       label: 'is not',              arity: 1, types: ['any'],
      sql: (col, vals) => `${col} <> ${literal(vals[0])}` },
    { id: 'contains',  label: 'contains',            arity: 1, types: ['text', 'any'],
      sql: (col, vals) => `${col} LIKE ${literal('%' + String(vals[0] ?? '') + '%')}` },
    { id: 'ncontains', label: 'does not contain',    arity: 1, types: ['text', 'any'],
      sql: (col, vals) => `(${col} NOT LIKE ${literal('%' + String(vals[0] ?? '') + '%')} OR ${col} IS NULL)` },
    { id: 'starts',    label: 'starts with',         arity: 1, types: ['text', 'any'],
      sql: (col, vals) => `${col} LIKE ${literal(String(vals[0] ?? '') + '%')}` },
    { id: 'ends',      label: 'ends with',           arity: 1, types: ['text', 'any'],
      sql: (col, vals) => `${col} LIKE ${literal('%' + String(vals[0] ?? ''))}` },
    { id: 'gt',        label: 'greater than (>)',    arity: 1, types: ['number', 'any'],
      sql: (col, vals) => `${col} > ${numberOrLiteral(vals[0])}` },
    { id: 'lt',        label: 'less than (<)',       arity: 1, types: ['number', 'any'],
      sql: (col, vals) => `${col} < ${numberOrLiteral(vals[0])}` },
    { id: 'gte',       label: 'greater than or equals (>=)', arity: 1, types: ['number', 'any'],
      sql: (col, vals) => `${col} >= ${numberOrLiteral(vals[0])}` },
    { id: 'lte',       label: 'less than or equals (<=)',    arity: 1, types: ['number', 'any'],
      sql: (col, vals) => `${col} <= ${numberOrLiteral(vals[0])}` },
    { id: 'between',   label: 'between',             arity: 2, types: ['number', 'any'],
      sql: (col, vals) => `${col} BETWEEN ${numberOrLiteral(vals[0])} AND ${numberOrLiteral(vals[1])}` },
    { id: 'empty',     label: 'is empty',            arity: 0, types: ['any'],
      sql: (col)       => `(${col} IS NULL OR ${col} = '')` },
    { id: 'nempty',    label: 'is not empty',        arity: 0, types: ['any'],
      sql: (col)       => `(${col} IS NOT NULL AND ${col} <> '')` },
    { id: 'in',        label: 'is one of',           arity: 'list', types: ['any'],
      sql: (col, vals) => `${col} IN (${vals.map(literal).join(', ')})` },
    { id: 'nin',       label: 'is not one of',       arity: 'list', types: ['any'],
